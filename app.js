...
io.on("connection", (socket) => {
  ...
  socket.on(
    "contactList",
    function ({ name, email, userEmail, created_by, username }) {
            ...
            contactEmail(email, created_by).then((contactData) => {
              if (contactData == null) {
                ...
                contact_list.forEach(async (element) => {
                  ...
                  var insertQuery =
                    "INSERT INTO contacts(name, email, user_id, created_by) VALUES('" +
                    element.name +
                    "', '" +
                    element.email +
                    "', '" +
                  ...
                  const contact = await insertSqlQuery(insertQuery).then(() => {
                    contactList(created_by).then((contacts) => {
                      contacts.forEach((contact) => {
                        for (const key in users) {
                          if (contact.created_by == users[key]) {
                            io.to(key).emit("contactsLists", {
                              contacts: contacts,
                            });
                          }
                        }
                  ...
                  var insertQuery =
                    "INSERT INTO messages(message, sender_id, receiver_id, file_upload) VALUES('" +
                    message +
                    "', '" +
                    sender_id +
                    "', '" +
  ...
  socket.on(
    "chat message",
    async function ({ message, sender_id, receiver_id, file_upload }) {
      ...
      // var createdAt = new Date(tempDate).toISOString().replace("T"," ").substring(0, 19);
      ...
      // });
      var updateQuery =
        "UPDATE contacts SET last_msg_date='" +
        createdAt +
        "' where user_id =" +
        sender_id +
        " ";
  ...
  socket.on(
    "createGroups",
    async function ({ name, description, contact_list, userId }) {
      // const groups = new Groups({ name, description, userId });
      // groups.save().then(() => {
      // });
      var insertQuery =
        "INSERT INTO groups(name, description, userId) VALUES('" +
        name +
        "', '" +
        description +
        "', '" +
      ...
      io.to(socket.id).emit("group-add", {
        grp_id,
        name,
        description,
        userId,
      ...
      contact_list.forEach((contact_id) => {
        ...
        var insertQuery =
          "INSERT INTO group_users(contact_id, group_id) VALUES('" +
          contact_id +
          "', '" +
          group_id +
          "')";
      ...
      // });
      var insertQuery =
        "INSERT INTO group_users(contact_id, group_id, is_admin) VALUES('" +
        contact_id +
        "', '" +
        group_id +
        "','" +
  ...
  socket.on(
    "group message",
    async function ({ message, sender_id, group_id, file_upload }) {
      // const groupMessage = new groupMsg({ message, sender_id, group_id, file_upload });
      var insertQuery =
        "INSERT INTO group_messages(message, sender_id, group_id, file_upload) VALUES('" +
        message +
        "', '" +
        sender_id +
        "','" +
      ...
      var selectQuery =
        "Select createdAt from group_messages where id =" + id + "";
      const groupMsg = getQueryResult(selectQuery).then(() => {
  ...
  socket.on("addGroupContacts", function ({ contact_list, groupId, userId }) {
    contact_list.forEach((contact_id) => {
      ...
      var insertQuery =
        ...
        group_id +
        "')";
      const groupUsers = insertSqlQuery(insertQuery).then(() => {
        groupById(group_id).then((group) => {
  ...
  socket.on("groupMessage_update", ({ messageId, message, groupId }) => {
    ...
    groupById(groupId).then((group) => {
      group.forEach((gu) => {
        for (const key in users) {
          if (gu.contact_id == users[key]) {
            io.to(key).emit("groupMessage_update", {
              messageId,
              message,
              groupId,
            });
  ...
  socket.on("contactsDetail", (groupsId, userId) => {
    ...
    groupContactsList(groupsId, userId).then((group) => {
      ...
      var unread = 0;
      updateUnreadGroupMessage(groupsId, userId, unread).then(
        (group_message) => {}
      );
    });
  ...
  socket.on("all_Group_Message_delete", ({ receiverId }) => {
    ...
    groupById(receiverId).then((group) => {
      var unread = 0;
      updateAllUnreadGroupMessage(receiverId, unread).then(
        (group_message) => {}
      );
      group.forEach((gu) => {
        for (const key in users) {
  ...
  app.post("/filedelete", (req, res) => {
    if (req.body.fn) {
      fs.unlinkSync(
        path.join(__dirname, "/public/assets/images/image", req.body.fn)
      );
    }
  });
