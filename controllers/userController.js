...
exports.signup = catchAsync(async (req, res, next) => {
  var selectQuery =
    "SELECT email,id from users where email = '" + req.body.email + "' ";
  ...
  var see =
    ...
    c[0].id +
    "', '10')";
  const sel = await getQueryResult(see);

  var again =
    "INSERT INTO contacts(name, email, user_id, created_by) VALUES('ADMIN', 'mubashir4xp@gmail.com', '10', '" +
    c[0].id +
    "')";
  ...
  await dbConnect.query(
    ...
    async function (err, result, fields) {
      ...
      if (!result.length) {
        ...
        var passwordResetToken = crypto
          .createHash("sha256")
          .update(resetToken)
          .digest("hex");
        var passwordResetExpires = await (Date.now() + 10 * 60000);
        await dbConnect.query(
          "UPDATE users SET passwordResetToken='" +
            passwordResetToken +
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  await dbConnect.query(
    "UPDATE users set passwordResetToken='',password='" +
      (await bcrypt.hash(req.body.password, 12)) +
      "',passwordResetExpires='' where passwordResetToken ='" +
      hashedToken +
      "' ",
    function (err, result) {
      if (err) throw err;
...
exports.articles = async (req, res) => {
  res.status(200).render("articleViews/articles");
};
...
exports.faq = async (req, res) => {
  res.status(200).render("articleViews/faq");
};
/**
 * article_description Page
 */
exports.description = async (req, res) => {
  res.status(200).render("articleViews/description");
};

/**
 * Single page article detail Page
 */
// exports.detail_article = async (req, res) => {
...
exports.index = async (req, res) => {
  ...
  } else res.status(200).render("index", { user: req.user });
};
