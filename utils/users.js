...
async function contactList(userId) {
  ...
  var selectQuery =
    "Select DISTINCT contacts.name,contacts.email,contacts.user_id,contacts.created_by,users.image as userImg,users.createdAt,users.location,(SELECT message from messages WHERE sender_id = " +
    userId +
    " ORDER BY id DESC LIMIT 1) as message,(SELECT file_upload from messages WHERE sender_id = " +
    userId +
    " ORDER BY id DESC LIMIT 1) as file_upload,(SELECT createdAt from messages WHERE sender_id = " +
    userId +
    " ORDER BY id DESC LIMIT 1) as created_at from contacts LEFT JOIN users ON users.id = contacts.user_id where contacts.created_by = " +
    userId +
    " order by contacts.name ASC";
...
async function searchContactData(name, userId) {
  ...
  var selectQuery =
    ...
    " ORDER BY id DESC LIMIT 1) as file_upload,(SELECT createdAt from messages WHERE sender_id = " +
    userId +
    " ORDER BY id DESC LIMIT 1) as created_at from contacts LEFT JOIN users ON users.id = contacts.user_id where contacts.name LIKE '%" +
    name +
    "%' and contacts.created_by = " +
    userId +
    " order by contacts.name ASC";
  const users = await getQueryMultipleResult(selectQuery);
  return users;
...
async function messageSearchData(searchVal, user_id, receiverId) {
  ...
  // ]);
  var selectQuery =
    "Select messages.message, messages.sender_id, messages.receiver_id, messages.file_upload, messages.createdAt, users.id as user_id,users.name, users.image from messages INNER JOIN users ON users.id = messages.sender_id   where (messages.receiver_id =" +
    user_id +
...
async function receiverMessage(id) {
  ...
  // ]);
  var selectQuery =
    "Select messages.message, messages.sender_id, messages.receiver_id, messages.file_upload, messages.createdAt, users.id,users.name, users.image from messages JOIN users ON users.id = messages.sender_id where (messages.receiver_id =" +
    id +
    " or messages.sender_id =" +
    id +
    ")";
...
async function userJoin(userId) {
  ...
  // ]);
  var selectQuery =
    "Select DISTINCT contacts.name, contacts.email, contacts.user_id, contacts.created_by, contacts.last_msg_date, messages.unread as unreadMsg, users.createdAt, users.location, users.image as userImg from contacts LEFT JOIN users ON users.id = contacts.user_id LEFT JOIN messages ON messages.sender_id = contacts.user_id   where contacts.created_by =" +
    userId +
...
async function userMessage(id, user_id, receiverId, startm) {
  ...
  // ]);
  var selectQuery =
    "Select DISTINCT messages.message, messages.sender_id, messages.receiver_id, messages.file_upload, messages.createdAt, messages.updatedAt, users.id as user_id,messages.id as id, users.image, contacts.name from messages LEFT JOIN users ON users.id = messages.sender_id LEFT JOIN contacts ON contacts.user_id = messages.sender_id where (messages.receiver_id =" +
    id +
    " or messages.sender_id =" +
    id +
    ") order by messages.id desc Limit 10";
...
async function groupById(groupsId) {
  ...
  // ]);
  var selectQuery =
    "Select group_users.unread, group_users.is_admin, group_users.contact_id, group_users.group_id, users.id as user_id, users.name from group_users LEFT JOIN users ON users.id = group_users.contact_id  where group_users.group_id =" +
    groupsId +
    " ";
...
async function searchGroupData(name, userId) {
  ...
  // ]);
  var selectQuery =
    "Select group_users.unread, group_users.contact_id, group_users.group_id, groups.name, groups.description, groups.userId, groups.id from group_users LEFT JOIN groups ON groups.id = group_users.group_id where (groups.name LIKE '%" +
    name +
    "%') and (group_users.contact_id =" +
    userId +
    ") ";
  const contactList = await getQueryMultipleResult(selectQuery);
  return contactList;
...
async function groupSearchData(name, id) {
  ...
  // ]);
  var selectQuery =
    "Select group_messages.message, group_messages.sender_id, group_messages.group_id, group_messages.file_upload, group_messages.createdAt, users.name, users.image  from group_messages LEFT JOIN users ON users.id = group_messages.sender_id where  (group_messages.group_id =" +
    id +
    ") and (group_messages.message LIKE '%" +
    name +
    "%') order by group_messages.id desc LIMIT 10 ";
...
async function unreadGroupUser(groupsId) {
  ...
  var selectQuery =
    "Select * from group_users where group_id =" + groupsId + " ";
...
async function groupsMessage(id, startm = 0) {
  ...
  // ]);
  var selectQuery =
    "Select group_messages.message,group_messages.id, group_messages.sender_id, group_messages.group_id, group_messages.file_upload, group_messages.createdAt, users.name, users.image from group_messages LEFT JOIN users ON users.id = group_messages.sender_id where (group_messages.group_id =" +
    id +
    ") order by group_messages.id desc LIMIT 10 ";
  const groupMessage = await getQueryMultipleResult(selectQuery);
  return groupMessage;
...
async function groupMessageUpdate(id, message) {
  ...
  var updateQuery =
    "UPDATE group_messages SET message='" + message + "' where id =" + id + " ";
  const message_update = await updateSqlQuery(updateQuery);
  return message_update;
}
...
async function contactListByUser(userId) {
  ...
  // ]);
  var selectQuery =
    "Select DISTINCT group_users.unread, group_users.contact_id, groups.name, groups.description, groups.userId, groups.id as group_id from group_users INNER JOIN groups ON groups.id = group_users.group_id where group_users.contact_id =" +
    userId +
    " ";
  const contactList = await getQueryMultipleResult(selectQuery);
  return contactList;
...
async function groupDeleteMember(id, group_id) {
  ...
  var deleteQuery =
    "DELETE from group_users WHERE contact_id =" +
    id +
    " and group_id =" +
    group_id +
    " ";
...
async function notificationUpdate(id, notification) {
  ...
  var updateQuery =
    "UPDATE users SET notification=" + notification + " where id =" + id + " ";
  const message_update = await updateSqlQuery(updateQuery);
  return message_update;
}
...
async function notificationMutedUpdate(id, is_muted) {
  ...
  var updateQuery =
    "UPDATE users SET is_muted=" + is_muted + " where id =" + id + " ";
  const message_update = await updateSqlQuery(updateQuery);
  return message_update;
}
...
async function contactListByUserId(userId, created_by) {
  ...
  // ]);
  var selectQuery =
    "Select contacts.name, contacts.email, contacts.user_id, contacts.created_by, users.image as userImg, users.createdAt, users.location from contacts JOIN users ON users.id = contacts.user_id where (users.id =" +
    userId +
...
async function lastMessageShow(userId) {
  ...
  // ]);
  var selectQuery =
    "Select message,createdAt as created from messages where (receiver_id =" +
    userId +
    " or sender_id =" +
    userId +
...
async function searchData(name, userId) {
  ...
  // ]);
  var selectQuery =
    "Select contacts.name, contacts.email, contacts.user_id, contacts.created_by, users.image as userImg, users.createdAt, users.location, messages.message, messages.file_upload, messages.unread as unreadMsg, messages.createdAt as created_at from contacts JOIN users ON users.id = contacts.user_id JOIN messages ON messages.sender_id = contacts.user_id where (users.id =" +
    userId +
    ") and (messages.sender_id =" +
    userId +
    ") and (contacts.created_by =" +
    userId +
    ") and contacts.name LIKE '%" +
    name +
    "%' order by messages.id desc ";
  const users = await getQueryResult(selectQuery);
  return users;
